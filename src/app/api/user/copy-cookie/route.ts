  ...
  try {
    ...
    if (user.credits < COPY_COST) {
      return NextResponse.json(
        { success: false, error: `Créditos insuficientes. Necesitas ${COPY_COST} crédito(s), tienes ${user.credits}` },
        { status: 400 }
      );
    }
    ...
    // Verify cookie still works before giving it
    ...
    // Success — deduct credits (more expensive) and update cookie
